package soso.sosoproject.controller.exception;
...
public class ExceptionHadlingController implements ErrorController {
    ...
    private final String ERROR_404_PAGE_PATH = "/error/error-404-new";
    private final String ERROR_500_PAGE_PATH = "/error/error-500-new";
    private final String ERROR_403_PAGE_PATH = "/error/error-403-new";
    private final String ERROR_ETC_PAGE_PATH = "/error/error";


    @RequestMapping(value = "/error")
    public String handleError(HttpServletRequest request, Model model) { // 에러 코드를 획득한다.
        ...
        if (status != null) { // HttpStatus와 비교해 페이지 분기를 나누기 위한 변수
            ...
            logger.info("httpStatus : " + statusCode); // 404 error
            if (statusCode == 403){
                return ERROR_403_PAGE_PATH;
            }
            System.out.println(HttpStatus.NOT_FOUND.value());
            if (statusCode == HttpStatus.NOT_FOUND.value()) { // 에러 페이지에 표시할 정보
